<template>
    <el-autocomplete class="searchCustom" v-model="searchVaule" :fetch-suggestions="querySearchAsync" placeholder="请输入江河名称" @select="handleSelect" 
         popper-class="searchCustomPopper" :trigger-on-focus="false" @keyup.enter.native="handleSelect">
         <i class="el-icon-search el-input__icon searchBtn" slot="suffix" @click='handleSelect'></i>
        <template slot-scope="{ item }">
            <span class="name">{{ item.name }}</span>
            <span class="type" v-if="!item.noData">{{ $tools.renderCollection.waterType(item.type) }}</span>
        </template>
    </el-autocomplete>
</template>
...
export default {
    ...
    async created(){
        ...
        // }
        this.quickData = (await this.$http.get('/getWaterResourceData')).data;
    }
}
</script>
