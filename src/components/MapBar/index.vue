...
import $ from "jquery";
echarts.registerMap("chengdu", area);
export default {
  data() {
    return {
      myChart: ""
  ...
  methods: {
    initChart() {
      ...
      let option = {
        ...
        geo: {
          ...
          roam: false,
          zoom: 0.9, // 地图初始大小
          center: [104.117022, 30.588158], // 初始中心位置
          label: {
            emphasis: {
              show: false,
              areaColor: "#eee"
      ...
      setTimeout(this.renderEachCity(data), 0);
      var throttledRenderEachCity = this.throttle(this.renderEachCity(data), 0);
      ...
      this.myChart.on("click", e => {
        ...
        if (e.componentSubType == "bar") {
          ...
          divObj.id = "zhuzhuang";
          var divX = this.getMousePos()["x"];
          var divY = this.getMousePos()["y"];
          $(divObj)
            .css({
              width: 250,
              height: 180,
    ...
    renderEachCity(data) {
      ...
      echarts.util.each(data, (dataItem, idx) => {
        var geoCoord = geoCoordMap[dataItem.name];
        var coord = this.myChart.convertToPixel("geo", geoCoord);
