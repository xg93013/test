        ...
        >
          <template slot-scope="scope">
            ...
            <div v-else-if="item.prop === 'operation'">
              <div class="operation">
                <span
                  v-if="scope.row['taskStatus'] == constant.COMPILE||scope.row['taskStatus'] == constant.FAIL"
                  @click="handleEdit(scope.$index, scope.row)"
                  class="edit-data"
                >
                  <svg-icon iconClass="edit"></svg-icon>
                  <label>{{scope.row['taskStatus'] == constant.COMPILE ? '编制':'修改'}}</label>
                </span>
...
import { setStorage, getStorage, getTimeStr } from "@/utils/index.js";
    ...
    exportStatic() {
      ...
      if (this.selectRow.length == 0) {
        ...
        if (this.selectRow.length == 1) {
          fileName = `报告-${this.selectRow[0].enterpriseName}-${timeStr}.zip`;
        } else {
          fileName = `报告-${timeStr}.zip`;
        }
