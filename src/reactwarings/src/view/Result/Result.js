...
class Result extends React.Component {
    ...
    constructor(props) {
        ...
        this.state = {
            options: [],
            defaultValue: [],
            leftLists: ["网络餐饮", "餐厨垃圾", "等级评定", "农产品预警", "明厨亮灶", "乡村群宴"],
    ...
    async getTimes() {
        ...
        if (res) {
            ...
            let arr = [...res.data[0].data.month].reverse();
            ...
            this.setState({
                options,
                defaultValue: [res.data[0].years + "年", month]
            });
            this.getDatas([res.data[0].years + "年", month]);
        }
    }

    listChange(leftListNum) {
        if (this.state.leftListNum === leftListNum || leftListNum !== 0) {
            return false;
        }
            ...
            <div id="result" className="result">
                ...
                <div className="right">
                    <div className="right-box">
                        <All
                            ref="all"
                            getOtherDatas={this.getOtherDatas.bind(this)}
                            allDatas={allDatas}
                            num={leftListNum}
                        />
                        <Chart ref="chart" />
                        <Rtable
                            ref="table"
                            status={status}
                            tabs={tabs[leftListNum]}
                            defaultValue={defaultValue}
                            listName={leftLists[leftListNum]}
                        />
                    </div>
                </div>
            </div>
