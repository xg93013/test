<template>
  <div id="header">
    <div class="left" id="headerLeft">
      <el-popover popper-class="left-pop" v-model="imgRotate">
        <div class="pops">
          <div
            class="pop pop-arr"
            v-for="(item,index) in systemDatas.resources"
            :key="index"
            @click="goPage(item)"
          >
            <img
              class="logo-img"
              :src="item.iconPath?require('./images/'+getIcon(item.iconPath)+'.png'):''"
              alt
            >
            {{item.resName}}
        ...
        <div class="list" slot="reference">
          <img
            class="logo-img"
            @click.self="imgRotate?imgRotate=false:imgRotate=true"
          ...
          <img
            class="arr-img"
            @click.self="imgRotate?imgRotate=false:imgRotate=true"
            :class="{'arr-img-rotate': imgRotate}"
    ...
    <div class="right">
      <div class="r1" v-show="path=='home'" @mouseleave="clearDatas">
        <div class="search-box">
          <el-input
            v-model="searchWord"
            @keyup.native="getDatas"
            placeholder="企业名称"
            class="search"
          />
          <div v-show="searchLists.length" class="search-lists">
            <div
              v-for="(item,index) in searchLists"
              :key="index"
              @click="goBusiness(item.enterpriseName,item.enterpriseId)"
...
const { ENTERPRISE_SEARCH, SYSTEM_POWER, OIDC_LOGOUT, HOME_PAGE } = apis;
export default {
  data() {
    return {
      y: "",
  ...
  methods: {
    ...
    async getDatas() {
      if (this.searchWord) {
        let res = await http.get(ENTERPRISE_SEARCH + this.searchWord);
        if (res) {
