        ...
        <div class="content">   
            <div class="intro part_common box">
                ...
                <div class="exponentCircleBox">
                    <div class="exponentCircleArea">
                        ...
                        <el-progress type="circle" :width='100' color="rgba(1, 255, 255,1)" :show-text="false" :percentage="currentData.pollution_score ? currentData.pollution_score : 0" class="exponentCircle"></el-progress>
                        <div class="displayBox">
                            <div v-text="currentData.pollution_score" class="score"></div>
                            <div class="label">污染指数</div>
                        </div>
                    </div>
                    <div class="pTypeLabel" v-if="!$store.state.view.bigScreen" v-text="currentData.pollution_type ? currentData.pollution_type : '--'"></div>
                    <div class="comparison" v-if="$store.state.view.bigScreen">
                        <div class="pTypeLabel_b" v-text="currentData.pollution_type ? currentData.pollution_type : '--'"></div>
                        <p><span>环比：</span><span v-text="currentData.link_ratio" class="comparison_value" :class="[currentData.link_ratio && currentData.link_ratio.includes('+') ? 'pNum' : 'nNum']"></span></p>
                        <p><span>同比：</span><span v-text="currentData.same_ratio" class="comparison_value" :class="[currentData.same_ratio && currentData.same_ratio.includes('+') ? 'pNum' : 'nNum']"></span></p>
                    </div>
                </div>
                <!-- 大屏隐藏 -->
                <div v-if="!$store.state.view.bigScreen">
                    <p class="inline"><span>污染位置</span><span v-text="currentData.position || '--'" :title="currentData.position"></span></p>
                    <p><span>水资源名称</span><span v-text="currentData.name || '--'"></span></p>
                    <p><span>区域</span><span :title="currentData.region? currentData.region.toString():'--'" v-text="currentData.region? currentData.region.toString():'--'"></span></p>
                    <p><span>水资源类型</span><span v-text="currentData.water_type"></span></p>
            ...
            <div class="intro_base part_common" v-if="$store.state.view.bigScreen">
                ...
                <div class="content">
                    <p><span>水资源名称</span><span v-text="currentData.name || '--'"></span></p>
                    <p class="inline"><span>污染位置</span><span v-text="currentData.position || '--'" :title="currentData.position"></span></p>
                    <p><span>所属区域</span><span :title="currentData.region? currentData.region.toString():'--'" v-text="currentData.region? currentData.region.toString():'--'"></span></p>
                    <p><span>水资源类型</span><span v-text="currentData.water_type"></span></p>
                    <p><span>污染面积</span><span v-text="$tools.convertArea(currentData.area)"></span><span>&nbsp;km</span><sup>2</sup></p>
                </div>
            </div>
            <div class="part_common">
                ...
                <div class="feature_index_area">
                    <div class="index_items big" v-if="$store.state.view.bigScreen">
                        <div class="index_item" v-show="currentData.pollution_type=='水体富营养'">
                            <img :src="require(`@/image/index_chlorophyll.gif`)" alt="">
                            <div class="index_content">
                                <div>叶绿素a</div>
                                <div v-text="`(${currentData.feature_index ? currentData.feature_index.toFixed(2) : 0}mg/L)`"></div>
                            </div>
                        </div>
                        <div class="index_item"  v-show="currentData.pollution_type=='黑臭水体'">
                        ...
                        <div class="index_item" v-show="currentData.pollution_type=='泥沙污染'">
...
function refreshCharts(item){
    ...
    temp.value = [item.heichou_index, item.nisha_index, item.lvzao_index];
    var option = {
        series: [{
            data : [temp],
        }]
    ...
    var seriesOprigin = _.cloneDeep(_this.$store.state.pollutionLegendOptions.filter(i=>i.label == item.pollution_type));
