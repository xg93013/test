  ...
  <div id="Tracking">
    ...
    <div class="commonWidth top-search">
      <div class="fl">
        ...
        <el-select v-model="search.taskStatus" placeholder="任务状态">
          <el-option
...
import { setStorage, getTimeStr } from "@/utils/index.js";
      ...
      if (res) {
        ...
        for (let i = 0; i < data.length; i++) {
          this.tableData.push({
            ...
            taskBeginDate: data[i].beginDate ? data[i].beginDate : "-",
            taskEndDate: data[i].endDate ? data[i].endDate : "-",
            taskId: data[i].id
          });
        }
    ...
    exportStatic() {
      ...
      if (this.selectRow.length == 0) {
        ...
        if (this.selectRow.length == 1) {
          fileName = `跟踪指导记录-${this.selectRow[0].enterpriseName}-${timeStr}.zip`;
        } else {
          fileName = `跟踪指导记录-${timeStr}.zip`;
        }
