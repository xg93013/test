    ...
    <el-form label-width="200px" :model="form" :rules="formRules" ref="formRefs">
      <div class="commonWidth first">
        ...
        <div class="check-status">
          <el-form-item label="产品生产状态：" prop="enterpriseStatus">
            ...
            <div class="status-box">
              ...
              <el-select
                ...
                @remove-tag="removeSeasonDownTag"
          ...
          <div class="check-item" v-show="enterpriseType == 'CATERING_BUSINESS'">
            <el-form-item label="起始时间" prop="startTime" label-width="100px" style="float:left">
              <el-time-select
            ...
            <el-form-item prop="endTime" label="结束时间" label-width="100px" style="float:left">
              <el-time-select
...
export default {
  data() {
    let _this = this;
    let enterpriseNameValidator = (rule, value, callback) => {
    ...
    let enterpriseAddressValidator = (rule, value, callback) => {
      if (
    ...
    let enterpriseNoValidator = (rule, value, callback) => {
    ...
    let personValidator = (rule, value, callback) => {
    ...
    let produceNoValidator = (rule, value, callback) => {
    ...
    let riskLevelValidator = (rule, value, callback) => {
    ...
    let stopDateValidator = (rule, value, callback) => {
        ...
        if (!value) {
          ...
          if (_this.form.stopEndDate == "") {
            ...
            if (value >= _this.form.stopEndDate) {
              ...
              _this.$refs.formRefs.clearValidate("stopEndDate");
              callback();
            }
          }
        }
    ...
    let stopEndDateValidator = (rule, value, callback) => {
        ...
        if (!value) {
          ...
          if (_this.form.stopStartDate == "") {
            ...
            if (value <= _this.form.stopStartDate) {
              ...
              _this.$refs.formRefs.clearValidate("stopStartDate");
              callback();
            }
          }
        }
    ...
    let enterpeiseStatusValidator = (rule, value, callback) => {
      let flag = false;
    ...
    let checkDateValidator = (rule, value, callback) => {
    ...
    let startTimeValidator = (rule, value, callback) => {
      if (_this.enterpriseType == "CATERING_BUSINESS") {
        if (!value) {
          ...
          if (!_this.form.endTime) {
            ...
            if (value >= _this.form.endTime) {
              callback(new Error("起始时间不能大于结束时间！"));
            } else {
              _this.$refs.formRefs.clearValidate("endTime");
              callback();
            }
          }
        }
        ...
        if (!value) {
          ...
          if (!_this.form.startTime) {
            ...
            if (value <= _this.form.startTime) {
              ...
              _this.$refs.formRefs.clearValidate("startTime");
              callback();
            }
          }
        }
  ...
  methods: {
    ...
    async getDefaultData() {
      ...
      if (res) {
        ...
        if (res.data.code == 0) {
          ...
          let startTime = data.caterBeginTime
            ? data.caterBeginTime.split(" ")[1].split(":")
            : "";
          let endTime = data.caterEndTime
            ? data.caterEndTime.split(" ")[1].split(":")
            : "";
          ...
          this.form = {
            ...
            stopEndDate: data.shutdownEndDate
              ...
              : getTimeStr(true),
            startTime: startTime ? startTime[0] + ":" + startTime[1] : "",
            endTime: endTime ? endTime[0] + ":" + endTime[1] : "",
      ...
      if (this.enterpriseType == "FOOD_PRODUCTION") {
        ...
        if (treeCategory) {
          originData[0].children = treeCategory.data.data;
        }
      } else {
        this.riskLevelList = [
          {
        ...
        for (let i = 0; i < category.data.data.length; i++) {
          list.push({
            id: i + 1,
            label: category.data.data[i].name
          });
        }
        originData[0].children = list;
      }
      this.categoryTree.originData = originData;
    ...
    async submitData(type, resultForm, changeType, page) {
      ...
      if (
        this.enterpriseType == "CATERING_BUSINESS" &&
        resultForm.checkDate != ""
      ) {
        form.caterBeginTime =
          resultForm.checkDate + " " + resultForm.startTime + ":00";
        form.caterEndTime =
          resultForm.checkDate + " " + resultForm.endTime + ":00";
      }
      let res = await http.patch(TASKS_TASK, form);
      if (res) {
    ...
    removeVisibleTag(item) {
      let arr = deepClone(this.enterpriseCategory.normalCategory);
      arr.push(item);
      this.prevNormalCat = arr;
      this.normalVisibleChange(false);
    },
    removeSemiDownTag(item) {
      let arr = deepClone(this.enterpriseCategory.semiShutdownCategory);
      arr.push(item);
      this.prevSemiShutdownCat = arr;
      this.semiVisibleChange(false);
    },
    removeSeasonDownTag(item) {
      let arr = deepClone(this.enterpriseCategory.seasonShutdownCategory);
      arr.push(item);
      this.prevSeasonShutdownCat = arr;
      this.seasonVisibleChange(false);
    },
    removeShutDownTag(item) {
      let arr = deepClone(this.enterpriseCategory.shutdownCategory);
      arr.push(item);
      this.prevShutdownCat = arr;
      this.shutdownVisibleChange(false);
    },
