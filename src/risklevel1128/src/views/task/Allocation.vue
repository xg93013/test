      ...
      <el-table :data="tableData" border style="margin-top: 20px;">
        ...
        <el-table-column label="工作日期" prop="date" width="300" sortable sort-by="groupStart">
          <template slot-scope="scope">
            <div>
              <p v-show="enterpriseType!='CATERING_BUSINESS'">
              ...
              <p>
                <span class="aColor">{{enterpriseType == "CATERING_BUSINESS" ? '整改复查' : '跟踪指导'}}：</span>
...
export default {
  data() {
    ...
    let formulationStartValidator = (rule, value, callback) => {
      let label =
        _this.enterpriseType == "CATERING_BUSINESS" ? "整改复查" : "跟踪指导";
      if (!value) {
        callback(new Error("请选择日期！"));
      } else {
        if (value < _this.dateForm.trackingEnd) {
          callback(new Error(`日期不能小于${label}结束日期！`));
        } else {
          callback();
        }
      }
      ...
      if (!value) {
        ...
        if (_this.dateForm.trackingStart != "") {
          if (value > _this.dateForm.trackingStart) {
            callback(new Error(`日期不能大于${label}开始日期！`));
          } else {
            callback();
          }
        }
      ...
      dateList: [
        {
          label: "静态因素开始日期",
          prop: "staticStart",
          placeholder: "请选择静态因素开始日期",
          show: true
        },
        {
          label: "结束日期",
          prop: "staticEnd",
          placeholder: "请选择静态因素结束日期",
          show: true
        },
        {
          label: "体系检查开始日期",
          prop: "groupStart",
          placeholder: "请选择体系检查开始日期",
          show: true
        },
        {
          label: "结束日期",
          prop: "groupEnd",
          placeholder: "请选择体系检查结束日期",
          show: true
        },
        {
          label: "跟踪指导开始日期",
          prop: "trackingStart",
          placeholder: "请选择跟踪指导开始日期",
          show: true
        },
        {
          label: "结束日期",
          prop: "trackingEnd",
          placeholder: "请选择跟踪指导结束日期",
          show: true
        },
        {
          label: "编制报告开始日期",
          prop: "formulationStart",
          placeholder: "请选择编制报告开始日期",
          show: true
        },
        {
          label: "结束日期",
          prop: "formulationEnd",
          placeholder: "请选择编制报告结束日期",
          show: true
        },
        {
          label: "审批报告开始日期",
          prop: "ApprovalStart",
          placeholder: "请选择审批报告开始日期",
          show: true
        },
        {
          label: "结束日期",
          prop: "ApprovalEnd",
          placeholder: "请选择审批报告结束日期",
          show: true
        }
      ],
      checkFormRules: {},
  ...
  created() {
    ...
    if (this.enterpriseType == "CATERING_BUSINESS") {
      for (let i = 0; i < 2; i++) {
        this.personList[i].show = false;
        this.dateList[i].show = false;
        delete this.checkFormRules[this.personList[i].prop];
        delete this.dateFormRules[this.personList[i].prop];
      }
      this.personList[4].label = "整改复查组长";
      this.personList[4].title = "整改复查";
      this.personList[5].title = "整改复查";
      this.dateList[4].label = "整改复查开始日期";
    }
  },
  methods: {
    async getPlanData() {
      ...
      if (res) {
        ...
        this.planForm.planType = res.data.data.planType;
    ...
    async getDatas() {
      ...
      // if (this.enterpriseType == "CATERING_BUSINESS") {
      //   delete dateKeyArr["STATIC"];
      //   delete personKeyArr["STATIC"];
      // }
      ...
      if (this.taskType == "add") {
        let [
          ...
          http.get(
            ACCOUNTS + "/" + this.planForm.planType + "/STATIC_RISK/accounts"
          ),
          http.get(
            ACCOUNTS + "/" + this.planForm.planType + "/SPOT_RECORD/accounts"
          ),
          http.get(
            ACCOUNTS + "/" + this.planForm.planType + "/TRACK_GUIDE/accounts"
          ),
          http.get(
            ACCOUNTS + "/" + this.planForm.planType + "/REPORT_EDIT/accounts"
          ),
          http.get(
            ACCOUNTS +
              "/" +
              this.planForm.planType +
              "/REPORT_APPROVAL/accounts"
          )
        ]);
        if (staticrisk) {
          let list = [];
    ...
    backCheck(arr) {
      // console.log(arr);
      if (arr.length > 0) {
        this.settingTitle = "点击查看筛选状态";
      } else {
        this.settingTitle = "选择配置状态";
      }
      if (arr.length == 1) {
    ...
    getStatusCount() {
      ...
      for (let i = 0; i < this.totalData.length; i++) {
        ...
        for (let j = 0; j < this.personList.length; j++) {
          if (
            !this.tableData[i][this.personList[j].prop] &&
            this.personList[j].show
          ) {
            personFlag = false;
            break;
          } else {
        ...
        for (let k = 0; k < this.dateList.length; k++) {
          if (
            !this.tableData[i][this.dateList[k].prop] &&
            this.dateList[k].show
          ) {
            dateFlag = false;
            break;
          } else {
    ...
    changeAllDate() {
      this.$refs.dateFormRefs.validate(flag => {});
    }
  }
};
</script>
