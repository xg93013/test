    ...
    async getDefaultData() {
      ...
      if (allplan) {
        ...
        if (this.allPlanList.length > 0) {
          let index = 0;
          let taskSelectPlan = getStorage("taskSelectPlan");
          if (taskSelectPlan) {
            for (let j = 0; j < this.allPlanList.length; j++) {
              if (this.allPlanList[j].planName == taskSelectPlan[0].planName) {
                index = j;
                break;
              } else {
                continue;
          ...
          this.enterpriseType = this.allPlanList[index].planType;
      ...
      if (res) {
        ...
        for (let i = 0; i < data.length; i++) {
          this.tableData.push({
            ...
            taskBeginDate: data[i].assignDate ? data[i].assignDate : "-"
