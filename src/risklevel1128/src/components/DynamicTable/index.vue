<template>
  <div class="dynamic-table">
    ...
    >
            ...
            <div v-if="!moreCheck">
              <div v-if="type=='view'">
                ...
                <div v-else class="select-box">
                  ...
                  <span class="cColor" style="font-weight:bold;">{{Number(scope.row['calScore'])}}</span>
                </div>
              </div>
              <div v-else>
                <span v-if="['', null].includes(scope.row[scope.column.property])">-</span>
      ...
      <el-table-column
        :width="showAddTotal ? 200 : 180"
        v-if="moreCheck&&type=='view'"
        align="center"
        label="汇总"
      >
        <template slot-scope="scope">
          <div>
            ...
            <p>总分/得分：{{scope.row.totalScore}} / {{scope.row.riskScore+scope.row.addTotal}}</p>
    ...
    >
      ...
      >
        <!-- :class-name="'start-select'?item.prop=='checkProject'" -->
        <template slot-scope="scope">
          <div
  ...
  methods: {
    ...
    getScore() {
      if (!this.moreCheck) {
        ...
        score.riskScore =
          score.riskScore >= 60 && this.enterpriseType == "FOOD_PRODUCTION"
            ? 60
            : score.riskScore;
        score.totalScore = this.getDynamicTotalScore();
        ...
        for (let i = 0; i < originData.length; i++) {
          let key = originData[i].dimName;
          let arr = originData[i].dataList;
          let obj = this.getAdvanceTotalScore(
            originData[i].dataList,
            originData[i].dimValue
          );
        ...
        if (parent.dataList[i].id == node.id) {
          ...
          if (parent.dataList.length == 0) {
            for (let j = 0; j < this.resultTree.dataList.length; j++) {
              if (parent.dimValue == this.resultTree.dataList[j].dimValue) {
    ...
    selectDown(arr, checkList, paramsData) {
      ...
      // this.showTableData();
      this.originTree = deepClone(this.resultTree);
      this.setPrevStatus();
    },
    // 保存上一次操作状态
    setPrevStatus() {
      ...
      if (node.dataList && node.dataList.length != 0) {
        ...
        if (linkParent) {
          for (let j = 0; j < origin.length; j++) {
            if (node.linkId == origin[j].parentId) {
              arr.push({ ...origin[j] });
            }
          }
        } else {
    ...
    async getDatas(flag) {
      ...
      if (res) {
        ...
        for (let i = 0; i < datas.length; i++) {
          ...
          // 企业先进性检查
          if (this.moreCheck) {
            let scoreObj = this.getAdvanceTotalScore(
              datas[i].dataList,
              datas[i].dataName
            );
    ...
    setViewData() {
      ...
      if (this.moreCheck) {
        for (let i = 0; i < resultData.length; i++) {
          let scoreObj = this.getAdvanceTotalScore(
            resultData[i].dataList,
            resultData[i].dimValue
          );
          ...
          resultData[i].addTotal = scoreObj.total - scoreObj.selectTotal;
        }
      }
          ...
          if (!flag) {
            for (let k = 0; k < treeData[i].children.length; k++) {
              obj.dataList.push({
                ...
                nopass: "符合",
                remarks: "",
                calScore: this.moreCheck ? treeData[i].children[k].score : 0,
            ...
            for (let m = 0; m < list.length; m++) {
              ...
              if (inIndex == -1) {
                obj.dataList.push({
                  ...
                  nopass: "符合",
                  remarks: "",
                  calScore: this.moreCheck ? list[m].score : 0,
                  score: list[m].score
                });
              } else {
      ...
      if (!this.moreCheck) {
        for (let i = 0; i < originData.length; i++) {
          ...
          for (let j = 0; j < originData[i].dataList.length; j++) {
            ...
            outObj.totalScore = this.getOneScore(originData[i].dimValue);
          }
          data.push(outObj);
        }
      } else {
    ...
    getAdvanceTotalScore(arr, label) {
      // 单项目所有检查项总分
      let totalScore = this.getOneScore(label);
      let obj = {
        total: totalScore,
      ...
      let percent =
        obj.totalScore - obj.missTotal != 0
          ...
          : 0;
      obj.percent = percent;
      obj.totalScore = obj.totalScore - obj.missTotal;
      return obj;
    },
    getDynamicTotalScore() {
      let arr = this.treeObj.data;
      let score = 0;
      for (let i = 0; i < arr.length; i++) {
        if (arr[i].children.length > 0) {
          for (let j = 0; j < arr[i].children.length; j++) {
            score += Number(arr[i].children[j].score);
          }
        }
      }
      return score;
    ...
    getOneScore(name) {
      ...
      for (let i = 0; i < arr.length; i++) {
        if (name == arr[i].label) {
          for (let j = 0; j < arr[i].children.length; j++) {
            totalScore += arr[i].children[j].score;
          }
          break;
        } else {
          continue;
...
.dynamic-table .default-table td.start-select {
  background-color: rgb(255, 244, 140);
}
.el-table--enable-row-hover .el-table__body tr:hover > td.start-select {
  background-color: rgb(255, 244, 140);
}
.dynamic-table .default-table td.gray-select {
  background-color: #ccc;
}
.el-table--enable-row-hover .el-table__body tr:hover > td.gray-select {
  background-color: #ccc;
}
</style>