...
1、	**config文件夹webpack相关配置，一般情况不做修改，开发环境需要修改的地方如下**:
...
```js
    ...
    {...
```
修改服务器代理， 开发环境将target修改为开发服务器地址

2、	**public文件夹**
> public\index.html
对该文件夹中的index.html头部进行相关配置
...
3、**script文件夹，项目启动或打包相关配置，一般不做修改，需要做修改的地方如下**
 
start.js文件，修改端口号DEFAULT_PORT，默认为3000
> scripts\start.js
...
```js
    ...
    const DEFAULT_PORT = parseInt(process.env.PORT, 10) || 8877;
...
**style**： 公用css
**unit**：一些静态js
**view**：视图文件夹，里面存放相关视图模块，每个视图模块都有相应的images文件夹，js，css

**注意**：如何存在公用图片或文件引用，在src下创建images或对应目录
