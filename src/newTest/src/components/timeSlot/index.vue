<template>
	<el-popover popper-class="szl-time-pop" @after-leave="cancel" v-model="show" width="340" trigger="click">
		<div class="time-slot">
			<el-row>
				<el-col class="time-range" @click.native="timeRange('monthly')" :class="{active: active=='monthly'}" align="center" :span="8">月度</el-col>
				...
				<el-col class="time-range" @click.native="timeRange('year')" :class="{active: active=='year'}" align="center" :span="8">年度</el-col>
			</el-row>
			<el-row style="margin-top:10px;background: #2E4354;">
				<el-col align="center" @click.native="prev" class="time-year" :class="{'time-year-disable': !isPrev}" :span="6">
					上一年
				</el-col>
				<el-col class="time-year" style="font-size:16px;color:#C5E6FE" :span="12" align="center">
					{{chooseMsg.year}}
				</el-col>
				<el-col align="center" @click.native="next" class="time-year" :class="{'time-year-disable': !isNext}" :span="6">
					下一年
				</el-col>
			</el-row>
			<el-row style="margin-top:10px">
				<el-col v-show="active=='monthly'">
					<div class="mons" :class="{active:active=='monthly'&&num==index,'time-disable':chooseMsg.year==currentYear&&index>currentMonth}" @click="timeClick(item,index)" v-for="(item,index) in monthly">
						{{item}}
					</div>
				</el-col>
				<el-col v-show="active=='quarter'">
					<div class="quarters" :class="{active:active=='quarter'&&num==index,'time-disable':chooseMsg.year==currentYear&&index>currentQuarter}" @click="timeClick(item,index)" v-for="(item,index) in quarter">
						{{item}}
					</div>
				</el-col>
				<el-col v-show="active=='halfYear'">
					<div class="halfYear" :class="{active:active=='halfYear'&&num==index}" @click="timeClick(item,index)" v-for="(item,index) in halfYear">
						{{item}}
					</div>
				</el-col>
	...
	export default {
		...
		methods: {
			timeClick(item, index) {
				if(this.active=='monthly'&&this.currentYear==this.chooseMsg.year&&index>this.currentMonth){
					return false
				}
				if(this.active=='quarter'&&this.currentYear==this.chooseMsg.year&&index>this.currentQuarter){
					return false
				}
				if(item!="全年"){
			...
			init() {
				...
				this.activeMsg = {
					...
					msg: (time.getMonth() + 1) < 10 ? "0" + (time.getMonth() + 1) + "月" : time.getMonth() + 1 + "月"
				}
				for(let i = 1; i < 13; i++) {
			...
			prev() {
				if(this.chooseMsg.year > (this.currentYear - 10)) {
					this.chooseMsg.year--;
					if(!this.isNext) {
						this.isNext = true;
					}
...
<style lang="scss">
	...
	.szl-time-pop[x-placement^=bottom] .popper__arrow {
