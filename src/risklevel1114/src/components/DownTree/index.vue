    ...
    >
      <div class="tree-select">
        <div id="tree-list">
          <div class="down-left">
            <div class="tree-top">
              <el-input v-model="keyWord" class="tree-search" placeholder="请输入关键词">
              ...
              >
                <div class="custom-tree-node" slot-scope="{ node, data }">
                  <div v-if="!node.isLeaf">
                    <span>{{node.label}}({{getNodeLen(node)}}/{{node.childNodes.length}})</span>
                  </div>
                  <div v-else>
          ...
          <div class="down-right">
            <div class="tree-top">
              <div class="nums">已选{{selectLen}}/{{totalLen > 0 ? totalLen-1: 0}}</div>
              <span class="clear-empty" @click="reset(true)">清空</span>
            </div>
            <div class="tree-content">
              <div
                class="item"
...
export default {
    ...
    defaultData(a) {
      this.$nextTick(() => {
        ...
        for (let i = 0; i < a.length; i++) {
          this.$refs.treeRefs.setChecked(a[i].id, true, true);
          this.checkList.push({
            id: a[i].id,
            label: a[i].label,
            show: true
        ...
        this.$emit(
          "downTree",
          this.isLeaf ? this.checkList : this.emitData,
          this.selectLen
        );
      });
    }
  ...
  methods: {
    ...
    pushEmitData(node, parentId) {
      ...
      //   let allParent = this.getAllParent(this.data, node.id);
