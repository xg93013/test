  ...
  <div id="TaskAllocation">
    ...
    <div class="commonWidth top-search">
      <div class="fl">
        ...
        <el-select v-model="search.entStatus" placeholder="企业状态" class="search-items">
          <el-option
  ...
  methods: {
    ...
    async getDefaultData() {
      ...
      if (allplan) {
        ...
        // 生产企业检查 PRODUCTION  餐饮企业检查 CATERER
        // pathCode GEN_FOOD_PRODUCT 普通食品生产企业
        if (this.pathCode == "GEN_FOOD_PRODUCT") {
          plans = plans.filter(item => {
            return item.planType == "PRODUCTION";
          });
        } else {
          plans = plans.filter(item => {
            return item.planType == "CATERER";
          });
        }
        let uncheckedList = [];
        let checkedList = [];
    ...
    handleData(index, row) {
      ...
      setStorage("enterpriseType", row.taskType);
      this.$router.push({
        name: "结果填报",
        params: {
