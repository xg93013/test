...
const { TASKS_TASKACTION, ACCOUNTS, PLAN, TASKS } = apis;
export default {
  data() {
    var _this = this;
    var trackingStartValidator = (rule, value, callback) => {
      if (!value) {
    ...
    var formulationStartValidator = (rule, value, callback) => {
      if (!value) {
        callback(new Error("请选择日期！"));
      } else {
    ...
    var ApprovalStartValidator = (rule, value, callback) => {
      if (!value) {
        callback(new Error("请选择日期！"));
      } else {
    ...
    var groupEndValidator = (rule, value, callback) => {
      // console.log(value);
      // console.log(_this.dateForm.groupStart);
      if (!value) {
        callback(new Error("请选择日期！"));
      } else {
  ...
  methods: {
    ...
    async getDatas() {
      ...
      // console.log(getStorage("taskIdList"));
      ...
      //   }
      // }
      let [group, track, report, reportApproval] = await Promise.all([
        // 13 现场检查 SPOT_RECORD_GFP
        // 14 跟踪指导 TRACK_GUIDE_GFP
        // 15 报告编制 REPORT_EDIT_GPF
        // 16 报告审批 REPORT_APPROVAL_GPF
    ...
    async submit() {
      let lists = getStorage("taskIdList");
      let taskIds = lists.join(",");
      let res = await http.patch(TASKS + "/" + taskIds);
      if (res) {
