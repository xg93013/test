        ...
        <div class="check-form">
          ...
          <div class="check-item">
            <el-form-item label="区（市）县局执法人员：" prop="areaPerson">
              ...
              <!-- <el-select v-model="inputForm.areaPerson" placeholder="企业负责人签名" style="width:100%;">
                <el-option
                  v-for="(item ,index) in allPersonList"
                  :key="index+'areaPerson'"
                  :label="item.label"
                  @click.native="changePerson('areaPerson', item)"
                  :value="item.label"
                ></el-option>
              </el-select>-->
            </el-form-item>
...
const { TASKS_TASK, TASKS_TASKIMPORANT } = apis;
  ...
  methods: {
    async getDefaultData() {
      ...
      if (res) {
        if (res.data.code == 0) {
          ...
          this.productNo = data.foodLicense;
          this.classDetail = data.productCategory;
    ...
    getResult() {
      ...
      resultForm.importantItem = this.importantContent
        ...
        : "无";
      resultForm.importantItemDescribe = this.others;
      resultForm.importantItemRemark = this.remarks;
