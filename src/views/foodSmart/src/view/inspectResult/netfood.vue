...
export default {
    ...
    methods: {
        showTableData(){
            ...
            this.$refs.resultTable.getTableMsg(INSPECT_LIST+'/'+this.year+month, this.currentStatus);
        },
        filterData(type){
        ...
        async getAllStatus(){
            ...
            let res = await http.get(INSPECT_STATUS+'/'+this.year+month);
            if(res){
        ...
        initData(){
            let time = JSON.parse(sessionStorage.getItem('netfoodTime'));
            if(time){
                let year = time.year;
                let month = time.msg;
                this.year = year;
