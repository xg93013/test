  ...
  methods: {
    ...
    cancelSelect() {
      if (this.selectMarkerId) {
        this.markerArr[this.selectMarkerId].setStyle(
          new ol.style.Style({
    ...
    addEvents() {
      ...
      that.map.on("pointermove", function(event) {
        that.map.getTargetElement().style.cursor = that.map.hasFeatureAtPixel(
          event.pixel
        )
          ? "pointer"
          : "";
    ...
    debounces() {
      // console.log('1')
      debounce(function() {}, Date.parse(new Date()), false);
    }
  },
  mounted() {
    // this.initMap()
