<template>
    <div class="editMapCon" ref="editmap">
        ...
        <div class="filterAreas">            
            <div class="editGridBtns">
                ...
                <div class="editOneGrid" v-show="showEditPolygon">
                	<Button @click="addOnePolygon" class="otherBtn" :disabled="!canAddPolygon" >添加多边形</Button>
                	<Button @click="editOnePolygon" class="otherBtn" :disabled="!canEditPolygon" >修改多边形</Button>
                	<Button @click="delOnePolygon" class="otherBtn" :disabled="!canDelPolygon" >删除多边形</Button>
                	<Button @click="finishEditPolygon" class="otherBtn" :disabled="!showFinishBtn">完成编辑</Button>
                	<Button @click="cancelEditPolygon" class="otherBtn">取消编辑</Button>
                </div>
                
                <Button @click="deletePolygonGrid" class="otherBtn" v-show="showDelGrid">删除网格</Button>
        ...
        <!-- 网格信息 -->
        <Modal v-model="showAddGridModal" title="请输入网格信息" :mask-closable="false" :closable="false" width="320px" :footer-hide="true">
            <div>
                <Form ref="addGridForms" :model="addGridForm" :rules="isAddOrEdit=='add'?ruleValidate:editRuleValidate" :label-width="80">
                    ...
                    <FormItem>
                        ...
                        <Button  @click="handleAddGrid('addGridForms')">保存</Button>
                        <Button  @click="clearAllTempPolygon" style="margin-left: 8px">取消</Button>
                    </FormItem>
                </Form>
            </div>
        </Modal>
...
//  import {communicationService,utilService,modalService} from '@/libs/uBase';
	import mapJson from '@/assets/data/polygon.json';
    ...
    const color = {
        ...
        colors : ["#F9ED69","#F08A5D","#FF9A00","#6A2C70","#222831","#000CCA","#00ADB5","#3EC1D3","#FF9A00","#FF165D"],
        ...
        watch: {
            currentGridID(newV){
                if(newV != 0){
                    ...
                    if(!this.showEditPolygon){
                    	this.showDelGrid = true;
                    }
//                  this.canEditPolygon = true;
                    ...
                    this.showDelGrid = false;
                    this.canStartEdit = false;
//                  this.canEditPolygon = false;
                }
            },
            'addGridForm.communityId'(newV){
            	if(newV != ''){
            		this.canAddGrid = true;
            	}else{
            		this.canAddGrid = false;
...
//              })
            ...
            bindPolygonEvent(polygonObj){
                ...
                polygonObj.on("mouseover" ,function (e) {
                    ...
                    // 案发提示框跟随鼠标
                    mapVM.popModel.show = true;
                    if (e.pixel.y + mapVM.$refs.polygonPop.offsetHeight >= mapVM.$refs.editmap.offsetHeight) {
                        mapVM.popModel.tipPopStyle.top = mapVM.$refs.editmap.offsetHeight-mapVM.$refs.polygonPop.offsetHeight + "px";
                    } else {
                        mapVM.popModel.tipPopStyle.top = e.pixel.y + "px";
                    }
                    if (e.pixel.x + mapVM.$refs.polygonPop.offsetWidth >= mapVM.$refs.editmap.offsetWidth) {
                        mapVM.popModel.tipPopStyle.left = mapVM.$refs.editmap.offsetWidth - mapVM.$refs.polygonPop.offsetWidth + "px";
                    } else {
                	...
                	let currentGridId = e.target.getOptions().extData.gridId;
                	console.log(currentGridId);
                	console.log(mapVM.currentGridID)
                	if(mapVM.showEditPolygon){
                		if(currentGridId != mapVM.currentGridID){
                			mapVM.$Modal.warning({
	                            title: "警告",
	                            content: "选择的多边形不在同一网格!",
	                            onOk : function () {}
	                        })
                		}else{
                			mapVM.setCurrentState(e);
                		}
                	}else{
                		mapVM.setCurrentState(e);
                	}
...
//                  }
            ...
            setCurrentState(e){
            	this.currentGridID = e.target.getOptions().extData.gridId;
            	this.currentPolygonId = e.target.getOptions().extData.id;
            	this.currentGridName = e.target.getOptions().extData.gridName;
            	this.changeStrokeStyleCL({
                    opacity : color.selectedStorkeOpacity,
                    strokeStyle : "dashed",
                    fillOpacity: 0.8
                }, e.target, true);
...
//              this.editGrid();
				this.showEditPolygon = true;
				this.showDelGrid = false;
				this.getAllEditGridPolygon();
            },
            cancelEditPolygon(){
            	this.showEditPolygon = false;
            	this.showDelGrid = true;
            	
            	if(!this.isEmptyObject(this.editPolygonObj) || this.deletePolygon){
            		this.currentGridID = 0;
            		this.getAllPolygon();
            	}
            	this.resetEditPolygon();
            },
            // 删除网格
            deletePolygonGrid(){
                let mapVM = this;
...
//                      mapVM.editPolygonObj[id].close();
                ...
                console.log(mapVM.allEditPolygonArr)
...
//              this.canEditPolygon = false;
            ...
            resetEditPolygon(){
            	...
            	this.canEditPolygon = true;
            	this.canDelPolygon = true;
            	this.showEditPolygon = false;
            	this.showFinishBtn = false;
            	
            	this.addEditAddPolygon = [];
            	this.delPolygonIdArr = [];
            	
            	if(this.mouseTool&&this.mouseTool!=''){
            		this.mouseTool.close(true);
            		this.mouseTool = '';
            	}
            	if(!this.isEmptyObject(this.editPolygonObj)){
            		this.editPolygonObj[this.currentPolygonId].close();
            		this.editPolygonObj = {};
            	}
            },
            clearAllTempPolygon(){
            ...
            addOnePolygon(){
                ...
                map.plugin(["AMap.MouseTool"],function(){
	                ...
	                mapVM.mouseTool.on('draw',function(e){
	                    ...
	                    onePolygon.points = JSON.stringify(points);
	                    mapVM.addEditAddPolygon.push(onePolygon);
//	                    mapVM.addGridForm.children.push(onePolygon);
//	                    mapVM.allTempPolygon.push(e.obj);
	                })
                })
            ...
            editOnePolygon(){
            	...
            	map.plugin(["AMap.PolyEditor"],function(){
            		mapVM.showFinishBtn = true;
                    mapVM.editPolygonObj[pId] = new AMap.PolyEditor(map,allPolygon[pId].polygonObj);
                    mapVM.editPolygonObj[pId].open();
                    mapVM.editPolygonObj[pId].on('adjust',(e)=>{
...
//                      console.log(pathArr);
                        let points = [];
                        for(let i=0;i<pathArr.length;i++){
                            points.push([pathArr[i].lng, pathArr[i].lat]);
                        }
//                      console.log(points)
                        for(let j=0;j<mapVM.allEditPolygonArr.length;j++){
                        	if(pId == mapVM.allEditPolygonArr[j].id){
                        		mapVM.allEditPolygonArr[j].points = JSON.stringify(points);
                        		mapVM.allEditPolygonArr[j].operation = 'M';
                        		return
                        	}
                        }
                    })
            ...
            delOnePolygon(){
            	...
            	for(let j=0;j<mapVM.allEditPolygonArr.length;j++){
                	if(pId == mapVM.allEditPolygonArr[j].id){
                		mapVM.allEditPolygonArr[j].operation = 'D';
                		mapVM.delPolygonIdArr.push(pId);
                		mapVM.showFinishBtn = true;
                		mapVM.deletePolygon = true;
                		console.log(mapVM.allEditPolygonArr)
                        mapVM.maps.remove(mapVM.allPolygon[pId].polygonObj)
                		return
                	}
               	}
//          	mapVM.$Modal.confirm({
...
//              })
            ...
            finishEditPolygon(){
            	this.editGridForm.children = [];
            	this.canAddPolygon = true;
            	if(this.delPolygonIdArr.indexOf(this.currentPolygonId)>-1){
            		this.canEditPolygon = false;
            	}else{
            		this.canEditPolygon = true;
            	}
            	...
            	// 将新增多边形加入
//          	console.log(this.allEditPolygonArr);
            	if(this.addEditAddPolygon.length!=0){
            		this.allEditPolygonArr = this.allEditPolygonArr.concat(this.addEditAddPolygon)
            		this.addEditAddPolygon = [];
            	}
            	// 修改信息
            	this.editGridForm.name = this.currentGridName;
            	this.addGridForm.name = this.currentGridName;
            	this.editGridForm.id = this.currentGridID;
            	for(let i=0;i<this.allEditPolygonArr.length;i++){
            		if(this.allEditPolygonArr[i].operation != ''){
            			this.editGridForm.children.push(this.allEditPolygonArr[i])
            		}
            	}
            	console.log(this.editGridForm);
            },
            resetEditPolygonBtn(){
            	this.canAddPolygon = true;
            	this.canEditPolygon = true;
