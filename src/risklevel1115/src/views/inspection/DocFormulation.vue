  ...
  <div id="DocFormulation">
    ...
    <div class="commonWidth top-search">
      <div class="fl">
        ...
        <el-select v-model="search.taskStatus" placeholder="任务状态" class="search-items">
          <el-option
        ...
        >
          <template slot-scope="scope">
            <div v-if="item.prop === 'taskStatus'" class="status">
              ...
              <span v-if="scope.row[item.prop] == constant.COMPILE" class="orange">待编制</span>
              <span v-if="scope.row[item.prop] == constant.APPROVAL" class="blue">待审批</span>
              <span v-if="scope.row[item.prop] == constant.PASS" class="pass">审批通过</span>
              <span v-if="scope.row[item.prop] == constant.FAIL" class="anomaly">审批未通过</span>
            </div>
            <div v-else-if="item.prop === 'operation'">
              <div class="operation">

                <span v-if="scope.row['taskStatus'] == constant.COMPILE||scope.row['taskStatus'] == constant.FAIL" @click="handleEdit(scope.$index, scope.row)">
                  <svg-icon iconClass="edit" color="#0079FE"></svg-icon>
                  <label>修改</label>
                </span>
                ...
                <!--                  <label>任务详情</label>-->
                <!--                </span>-->
                <span v-else @click="handleView(scope.$index, scope.row)">
                  <svg-icon iconClass="view" color="#0079FE"></svg-icon>
                  <label>预览报告</label>
                </span>
              </div>
            </div>
...
<script>
  ...
  const COMPILE = 'UNREPORT'//待编制
  const APPROVAL = 'UNAPPROVE'//待审批
  const PASS = 'APPROVED'//审批通过
  const FAIL = 'APPROVEFAILED'//审批未通过
  export default {
    data() {
      return {
        constant: {
    ...
    methods: {
      async getDefaultData() {
        let allplan = await http.get(PLAN,{
          planType:this.planType==="GEN_FOOD_PRODUCT"?"FOOD_PRODUCTION":"CATERING_BUSINESS"
        })
        if (allplan) {
          ...
          if (this.allPlanList.length > 0) {
            let index = getStorage('planIndex')
      ...
      getTotalCount(item) {
        this.statusCount.allNum = item.report
        this.statusCount.noNum = item.unReported
        this.statusCount.okNum = item.reported
      },
      async getDatas() {
        this.tableData = []
        let params = {}
      ...
      exportStatic() {
        ...
        let fileName = '报告审批.zip'
        let url = EXPORT_BASE + EXPORT_REPORT
