  ...
  <div id="PlanFormulation">
        ...
        >
          <template slot-scope="scope">
            ...
            <div v-else-if="item.prop === 'planDetail'">
              <span @click="handleView(scope.$index, scope.row)" v-show="scope.row.planDetail" class="view">
                <svg-icon iconClass="view"></svg-icon>
                <span>预览</span>
              </span>
            </div>
            <div v-else-if="item.prop === 'programDetail'">
              <span @click="handleProgram(scope.$index, scope.row)" v-show="scope.row.programDetail" class="view">
                <svg-icon iconClass="view"></svg-icon>
                <span>预览</span>
              </span>
            </div>
            <div v-else-if="item.prop === 'taskDetail'">
              <span @click="openTaskList(scope.$index, scope.row)" v-show="scope.row.taskDetail" class="view">
                <svg-icon iconClass="view"></svg-icon>
                <span>预览</span>
              </span>
            </div>
            <div v-else-if="item.prop === 'dataQuery'">
              <span @click="handleMode(scope.$index, scope.row)" v-show="scope.row.dataQuery" class="view">
                <svg-icon iconClass="search"></svg-icon>
                <span>查询</span>
              </span>
            </div>
            <div v-else-if="item.prop === 'analysis'">
              <span @click="handleAnalysis(scope.$index, scope.row)" v-show="scope.row.analysis" class="view">
                <svg-icon iconClass="view"></svg-icon>
                <span>预览</span>
              </span>
            </div>
            <div v-else-if="item.prop === 'analysisReport'">
              <span @click="handleReport(scope.$index, scope.row)" v-show="scope.row.analysisReport" class="view">
    ...
    <el-dialog title="选择任务" :visible.sync="taskVisible">
      <div class="header-search">
        <div class="search"><span>任务名称：</span><el-input placeholder="请输入任务名称" v-model="taskNameSearch" clearable></el-input><el-button type="primary" @click="getTaskList">查询</el-button></div>
      </div>
      <div>
        <el-checkbox-group v-model="taskSel" class="taskSel" @change="taskSelChange">
        <el-table ref="taskTable" :data="taskData" border highlight-current-row @row-dblclick="toTaskDetail" @current-change="taskRowChange" max-height="500">
...
export default {
  ...
  methods: {
    ...
    getTaskList() {
      this.taskSel = [];
      let params = {pageNo: this.page1.currentPage, pageSize: this.page1.pageSize};
    ...
    getParams() {
      let params = {pageNo: this.page.currentPage, pageSize: this.page.pageSize};
    ...
    retOperStatus(status) {
      let obj = {};
      switch(status) {
        case 'AUTHORIZED': obj = {planDetail: true}; break; // 待分配
        case 'ACTION': obj = {planDetail: true, taskDetail: true, dataQuery: true}; break; // 计划执行
        case 'APPROVE': obj = {planDetail: true, taskDetail: true, dataQuery: true, analysisReport: true}; break; // 计划完成
      }
      return obj;
    },
