<template>
    <div class="charts">
        ...
        <div class="cdmaps-lenged">
            <span class="a" :class="{'lenged-color': isFullScreen}">违规率高</span>
            <span class="b" :class="{'lenged-color': isFullScreen}">违规率低</span>
            <div v-for="(item,index) in lengedColor" :key="index" :style="{background: item}">
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        getAreaPoint() {
            ...
            //				let zone = this.$store.state.userMsg.zone;
            this.areaPoint = [];
            cd.features.forEach((item, index) => {
                this.areaPoint[item.properties.name] = item.properties.cp;
            });
            //				if(zone) {
            //					cd.features.forEach(item => {
            //						if(item.properties.name == zone) {
            //							this.mapMsg.features = [item]
            //						}
            //					})
            //				}
        ...
        changeData() {
            ...
            this.myOption.series = [
                ...
                },
                {
                    ...
                    //																		let num = e.value[2];
                    //																		if(num >= 0 && num <= 20) {
                    //																			return lengedColor[4]
                    //																		} else if(num > 20 && num <= 40) {
                    //																			return lengedColor[3]
                    //																		} else if(num > 40 && num <= 60) {
                    //																			return lengedColor[2]
                    //																		} else if(num > 60 && num <= 80) {
                    //																			return lengedColor[1]
                    //																		} else if(num > 80 && num <= 100) {
                    //																			return lengedColor[0]
                    //																		}
                    //																	}
                    //																}
                    //															},
