<template>
  <div id="home">
    <div
      id="header"
      :class="{'is-full-screen': leftSize==0&&path=='comAnalysis','is-full-screen-risk':leftSize==0&&path=='riskAnalysis'}"
    >
      <div class="risk-full-bg" v-show="leftSize==0&&path=='riskAnalysis'"></div>
      <span>{{userMsg.orgName}}</span>
      <!--<img v-show="leftSize==0" src="../images/namea.png" alt="" />-->
      <div class="userName" v-show="leftSize>0">
        <span v-if="userMsg.tenantName">
          {{userMsg.tenantName}}
          <br />
        </span>
        {{userMsg.loginName}}
    ...
    <div id="sidebar" v-show="leftSize>0">
      <div class="lists">
        <template v-for="(item,index) in sideBars">
          <router-link
            v-if="!item.children&&userMsg.permissions.includes(item.jur)"
            :key="index"
            :to="{path: item.path}"
            class="list"
            :class="path==item.path?item.path+'a':item.path"
          >
            {{item.name}}
            <span
              class="full-screen"
              v-if="item.home&&path=='comAnalysis'"
              @click="fullScreen"
            >全屏</span>
            <span class="full-screen" v-if="item.risk&&path=='riskAnalysis'" @click="fullScreen">全屏</span>
          </router-link>
          <div
            v-if="item.children&&userMsg.permissions.includes(item.jur)"
            class="list"
            :class="['mt','elm','bd'].includes(path)?item.path+'a':item.path"
            :key="index"
          ...
          >
            ...
            <i
              ...
              :class="{'arrow-rotate': item.height>0}"
            ></i>
          </div>
          <div
            :key="'a'+index"
    ...
    <div
      ...
      :class="{'is-full-screen': leftSize==0,'is-full-screen-risk':leftSize==0&&path=='riskAnalysis'}"
    >
      <keep-alive>
        <router-view v-if="$route.meta.keepAlive&&conShow" @init="init"></router-view>
      </keep-alive>
      <router-view v-if="!$route.meta.keepAlive&&conShow" @init="init"></router-view>
    ...
    <el-dialog
      ...
      title="修改密码"
      id="changePassword"
      :visible.sync="resetPsd.dialogShow"
      width="600px"
      :before-close="psdClose"
    >
      <el-form
        v-loading="resetPsd.loading"
      ...
      >
        <el-form-item style="margin-top:20px" label="旧密码" prop="oldPassword">
          <el-input
            ...
            type="password"
            placeholder="请输入旧密码"
            @keyup.enter.native="psdConfirm"
            v-model="resetPsd.oldPassword"
          />
        </el-form-item>
        ...
        <el-form-item label="确认密码" prop="confirmPassword">
          <el-input
            ...
            placeholder="请确认新密码"
            @keyup.enter.native="psdConfirm"
            v-model="resetPsd.confirmPassword"
          />
        </el-form-item>
...
const { LOGOUT, ACL, PASSWORD, REGION_JSON } = apis;
export default {
  data() {
  ...
  watch: {
    $route(to, from) {
      this.path = to.name;
      if (this.path != "account" && this.path != "riskAnalysis") {
        this.$store.commit("changeLoading", true);
      } else {
        this.$store.commit("changeLoading", false);
      }
...
#sidebar {
  ...
  .router-link-active {
    background-color: #276ab0 !important;
    color: #fff !important;
  }
}
...
#header.is-full-screen-risk {
  ...
  .risk-full-bg {
    ...
    top: 70px;
    background-image: url("../images/riskBga.png");
    background-size: cover;
  }
  .out-full {
    background: #313c46;
